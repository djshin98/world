...
function aRect(turnPlane, properties, bcompleted) {
    ...
    return turnPlane.reduce((prev, p, i, buffer) => {
        ...
        ret = ret.concat(calc.annotationOnLine(a, aname, 0.5, { x: distance / 2, y: 0 }, { x: -distance / 2, y: 0 }, true));
        ret = ret.concat(calc.annotationOnLine(a, aname, 0.5, { x: -distance / 2, y: 0 }, { x: -distance / 2, y: p[1].y }));
        ret = ret.concat(calc.annotationOnLine(a, aname, 0.5, { x: -distance / 2, y: p[1].y }, { x: distance / 2, y: p[1].y }, true));
