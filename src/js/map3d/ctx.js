var CTX = {
    ...
    r2d : function(r){ 
        if( CTX.debug ){ console.log("r2d : "); console.dir(r); }
    ...
    r2c : function(r){ 
        if( CTX.debug ){ console.log("r2c : "); console.dir(r); }
        return Cesium.Cartesian3.fromRadians(r.longitude,r.latitude,r.height); },
    r2w : function(r){ 
        if( CTX.debug ){ console.log("r2w : "); console.dir(r); }
        return CTX.c2w(CTX.r2c(r)); },
    d2r : function(d){ 
        if( CTX.debug ){ console.log("d2r : "); console.dir(d); }
    ...
    d2c : function(d){ 
        if( CTX.debug ){ console.log("d2c : "); console.dir(d); }
        return CTX.r2c(CTX.d2r(d)); },
    d2w : function(d){ 
        if( CTX.debug ){ console.log("d2w : "); console.dir(d); }
        return CTX.c2w(Cesium.Cartesian3.fromDegrees(d.longitude,d.latitude));},
    c2r : function(c){ 
        if( CTX.debug ){ console.log("c2r : "); console.dir(c); }
        return Cesium.Cartographic.fromCartesian(c); },
    c2d : function(c){ 
        if( CTX.debug ){ console.log("c2d : "); console.dir(c); }
        return CTX.r2d(CTX.c2r(c)); },
    c2w : function(c){ 
        if( CTX.debug ){ console.log("c2w : "); console.dir(c); }
        return Cesium.SceneTransforms.wgs84ToWindowCoordinates(CTX.viewer.scene, c); },
    w2r : function(x,y){ 
        if( CTX.debug ){ console.log("w2r : "); console.log("x="+x+" ,y="+y); }
        return CTX.c2r(CTX.w2c(x,y)); },
    w2d : function(x,y){ 
        if( CTX.debug ){ console.log("w2d : "); console.log("x="+x+" ,y="+y); }
